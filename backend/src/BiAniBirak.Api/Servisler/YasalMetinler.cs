...
Gizlilikle ilgili her türlü soru ve talebiniz için uygulama içindeki KVKK bölümünden bize ulaşabilirsiniz. Başvurular en geç 30 gün içinde yanıtlanır.";
    ...
    // ---- ODEME KAPSAMI ----
    //
    // Mesafeli Satis Sozlesmesi ve On Bilgilendirme Formu, 6502 sayili Tuketicinin
    // Korunmasi Hakkinda Kanun ile Mesafeli Sozlesmeler Yonetmeligi uyarinca ZORUNLUDUR.
    // Internetten yapilan her satis "mesafeli sozlesme"dir.
    //
    // AYRI KAPSAM ("odeme") olarak eklendi - kayit aninda degil, ODEME aninda onaylanir.
    // Kayit olan herkes musteri degildir; yalnizca odeyen musteri olur.
    public const string OnBilgilendirmeMetni = @"Ön Bilgilendirme Formu

Bu form, 6502 sayılı Tüketicinin Korunması Hakkında Kanun ve Mesafeli Sözleşmeler Yönetmeliği uyarınca, siparişinizi onaylamadan önce bilgilendirilmeniz amacıyla sunulmaktadır.

1. HİZMET SAĞLAYICI

BiAnıBırak
İletişim: uygulama içindeki KVKK/destek bölümü
...
Dilek toplamak, defteri kurmak, düzenlemek ve tamamını ekranda önizlemek ÜCRETSİZDİR. Ücret yalnızca baskıya hazır dosyanın indirilmesi içindir.

3. FİYAT

Güncel fiyat ödeme ekranında gösterilir. Fiyata tüm vergiler dahildir. Ek teslimat veya kargo ücreti yoktur (dijital teslimat).

4. ÖDEME ŞEKLİ

Havale/EFT ile banka hesabımıza ödeme yapılır. Ödemenizin açıklama kısmına, size verilen referans kodunu yazmanız zorunludur. Kod yazılmayan ödemeler eşleştirilemez.

5. TESLİMAT

Teslimat DİJİTALDİR ve ANINDA yapılır: ödemeniz onaylandığı anda indirme yetkiniz açılır. Fiziksel bir teslimat yoktur.

Ödemeniz genellikle birkaç saat içinde onaylanır. Onaylandığında bildirim alırsınız.

6. CAYMA HAKKI - ÖNEMLİ

Mesafeli Sözleşmeler Yönetmeliği'nin 15/1-ğ maddesi uyarınca, ELEKTRONİK ORTAMDA ANINDA İFA EDİLEN ve tüketiciye anında teslim edilen gayrimaddi mallara ilişkin sözleşmelerde CAYMA HAKKI BULUNMAMAKTADIR.

Ödemeniz onaylandığı ve indirme yetkiniz açıldığı anda hizmet ifa edilmiş sayılır. Bu nedenle cayma hakkınız yoktur.

İndirme yetkisi kazandığınız hâlde defterinizi indirmemeniz, hizmetin ifa edilmediği anlamına gelmez. Yetki size açılmıştır; kullanıp kullanmamak sizin tercihinizdir.
...
Bu nedenle: indirdiğiniz dosyayı MUTLAKA YEDEKLEYİNİZ.

8. UYUŞMAZLIK

Tüketici, şikâyet ve itirazları için Tüketici Hakem Heyetlerine veya Tüketici Mahkemelerine başvurabilir.";

    public const string MesafeliSatisMetni = @"Mesafeli Satış Sözleşmesi

MADDE 1 - TARAFLAR

SATICI: BiAnıBırak
ALICI: Uygulamaya kayıtlı, ödeme işlemini gerçekleştiren kullanıcı
...
Ödeme havale/EFT ile yapılır. ALICI, ödemenin açıklama kısmına kendisine verilen referans kodunu yazmakla yükümlüdür.

Referans kodu yazılmayan veya tutarı eksik olan ödemeler eşleştirilemez ve indirme yetkisi açılmaz. Bu durumda ALICI, SATICI ile iletişime geçmelidir.

MADDE 5 - İFA VE TESLİM

Ödeme onaylandığı anda indirme yetkisi açılır ve hizmet İFA EDİLMİŞ sayılır.

ALICI'nın indirme yetkisini kullanmaması (defteri indirmemesi), hizmetin ifa edilmediği anlamına gelmez.

MADDE 6 - CAYMA HAKKI

Mesafeli Sözleşmeler Yönetmeliği m.15/1-ğ uyarınca, elektronik ortamda anında ifa edilen ve tüketiciye anında teslim edilen gayrimaddi mallara ilişkin sözleşmelerde CAYMA HAKKI YOKTUR.

Hizmet, ödemenin onaylanmasıyla anında ifa edildiğinden ALICI'nın cayma hakkı bulunmamaktadır.
...
ALICI, indirdiği dosyayı yedeklemekle yükümlüdür. İmha sonrası dosyanın kaybı nedeniyle SATICI'dan herhangi bir talepte bulunulamaz.

MADDE 8 - SORUMLULUK SINIRI

SATICI, ALICI'ya son gün yaklaşırken hatırlatma bildirimleri gönderir. Ancak bu bildirimlerin ALICI tarafından görülmesini veya okunmasını sağlama yükümlülüğü SATICI'da değildir.

ALICI'nın bildirim izinlerini kapatmış olması, e-postasını kontrol etmemesi veya uygulamaya girmemesi nedeniyle süreyi kaçırması hâlinde sorumluluk ALICI'ya aittir.

MADDE 9 - İADE

Hizmet anında ifa edildiğinden ve cayma hakkı bulunmadığından, kural olarak iade yapılmaz.

Ancak SATICI'dan kaynaklanan teknik bir arıza nedeniyle ALICI hizmetten hiç yararlanamamışsa (indirme yetkisi açılmamış veya dosya üretilememişse), ödeme iade edilir.

MADDE 10 - YETKİLİ MAHKEME

İşbu sözleşmeden doğan uyuşmazlıklarda Konya Tüketici Hakem Heyetleri ve Konya Mahkemeleri yetkilidir.

ALICI, ödeme işlemini onaylamakla işbu sözleşmenin tüm maddelerini okuduğunu, anladığını ve kabul ettiğini beyan eder.";
    ...
    public static async Task SeedAsync(BiAniBirakDbContext db, CancellationToken ct = default)
    {
        ...
        // ODEME KAPSAMI - odeme aninda onaylanir (kayit aninda DEGIL).
        // Kayit olan herkes musteri degildir; yalnizca odeyen musteri olur.
        if (!mevcut.Contains("on_bilgilendirme"))
            eklenecek.Add(Olustur("on_bilgilendirme", "Ön Bilgilendirme Formu",
                OnBilgilendirmeMetni, kapsam: "odeme", zorunlu: true, sira: 1));

        if (!mevcut.Contains("mesafeli_satis"))
            eklenecek.Add(Olustur("mesafeli_satis", "Mesafeli Satış Sözleşmesi",
                MesafeliSatisMetni, kapsam: "odeme", zorunlu: true, sira: 2));
