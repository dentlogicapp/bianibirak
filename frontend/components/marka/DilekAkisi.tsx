...
// kalem soldan saga yazar gibi, sonsuz donguyle. Path olarak public'ten yuklenir.
export function DilekAkisi() {
  ...
  useEffect(() => {
    ...
    const olcMax = () =>
      setMaxw(Math.min(typeof window !== "undefined" ? window.innerWidth - 56 : 640, 640));
    olcMax();
    window.addEventListener("resize", olcMax);
    let iptal = false;
    ...
    return () => {
      iptal = true;
      window.removeEventListener("resize", olcMax);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setI((p) => (p + 1) % dilekler.length), bekle);
    return () => clearTimeout(t);
  }, [i, dilekler, sure]);

  return (
    <div className="flex min-h-[54px] items-center justify-center overflow-hidden">
      {aktif && (
        <div
          key={i}
          className="dilek-sar yaz"
          style={{ "--tw": `${w}px`, "--sure": `${sure}ms` } as React.CSSProperties}
        >
          ...
          <span className="dilek-kalem" aria-hidden="true">
            <svg viewBox="0 0 32 46" xmlns="http://www.w3.org/2000/svg">
              <g transform="rotate(38 16 23)">
                <path d="M16 4 L21.5 29 L16 37 L10.5 29 Z" fill="#C4A25E" stroke="#8A6B33" strokeWidth="0.9" />
