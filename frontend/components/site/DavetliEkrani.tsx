...
import { DavetliKarsilama } from "@/components/site/DavetliKarsilama";
...
// DAVETLI EKRANI - davetlinin baglantiyi actiginda gordugu SAYFANIN TAMAMI.
//
// TEK DOGRULUK KAYNAGI. Uc turdur ayni hatayi duzelttik: onizleme, gercek ekrani
// "taklit eden" ayri bir dizgiydi ve her seferinde ayristi (once film seridi yoktu,
...
//  X-Frame-Options: DENY basligina takildi. O baslik DOGRU bir guvenlik onlemi -
//  clickjacking'e karsi korur. Onizleme icin guvenlik gevsetilmez; mimari duzeltilir.)
//
// SALT MOD (onizleme): alanlar pasif, gonderim yok, sozluk indirilmez. Gorunum
// birebir aynidir - cunku ayni bilesendir.

// ILISKI TIPLERI.
// Deftere iyelik ekiyle basilir: "Musa'nin Okul Arkadasi".
// Detay alani opsiyoneldir; doldurulursa deftere O yazilir - davetlinin kendi
// cumlesi, sablon bir etiketten her zaman daha degerlidir.
...
export function DavetliEkrani({
  ...
  veri: KatkiKarsilama;
  onGonderildi: () => void;
  // SALT MOD: panel onizlemesi. Gorunum birebir aynidir; yalniz ETKILESIM kapalidir.
  salt?: boolean;
}) {
  ...
  useEffect(() => {
    if (salt) return; // onizlemede sozluk indirilmez - gereksiz 100 KB
    let iptal = false;
  ...
  function onizlemeyeGec(e: React.FormEvent) {
    e.preventDefault();
    if (salt) return; // onizlemede gonderim YOK
    setHata("");
  ...
  async function fotoSecildi(e: React.ChangeEvent<HTMLInputElement>) {
    if (salt) return; // onizlemede yukleme YOK
  ...
  return (
    <main
      className={`flex flex-col items-center bg-parsomen px-6 py-12 ${
        salt ? "" : "min-h-screen"
      }`}
      // SALT MOD: sayfa bir GORUNTUdur - tiklanmaz, yazilmaz. Gorunum birebir
      // aynidir; degisen YALNIZ etkilesimdir.
      style={salt ? { pointerEvents: "none", userSelect: "none" } : undefined}
    >
      <div className="w-full max-w-md">
        ...
        <div className="overflow-hidden rounded-3xl border border-ayrac bg-yuzey">
          <DavetliKarsilama
            ciftAdi={ciftAdi}
            karsilama={karsilama}
            promptMetni={veri.prompt_metni}
            gorseller={veri.gorseller}
            sayacAktif={veri.sayac_aktif}
            etkinlikTarihi={veri.etkinlik_tarihi}
            sayacAktifCumle={veri.sayac_aktif_cumle}
            sayacBittiCumle={veri.sayac_bitti_cumle}
          />
...
export function TesekkurEkrani({ ciftAdi, tur }: { ciftAdi: string; tur: string }) {
  const turAdi =
